 ...
 * Comprehensive threshold creation and editing interface with template management
 */
...
interface ThresholdWizardStep {
  label: string;
  optional?: boolean;
}

const WIZARD_STEPS: ThresholdWizardStep[] = [
  { label: 'Basic Information' },
  { label: 'Threshold Type & Configuration' },
  { label: 'Advanced Settings', optional: true },
  { label: 'Preview & Test', optional: true }
];

export const ThresholdConfiguration: React.FC<ThresholdConfigurationProps> = ({
  metricId,
  metricName,
  thresholds,
  templates,
  ...
  const filteredThresholds = useMemo(() => {
    let filtered = thresholds.filter(threshold => {
      ...
      if (filterText) {
        ...
        if (!threshold.name.toLowerCase().includes(searchText) &&
            ...
            !threshold.tags.some(tag => tag.toLowerCase().includes(searchText))) {
          return false;
        }
      }
    ...
    return filtered;
  }, [thresholds, filterText, filterType, filterSeverity, filterEnabled, sortBy, sortOrder]);
  ...
  const handleTestThreshold = useCallback(async (thresholdId: string) => {
    setLoading(true);
    try {
      const result = await onThresholdTest(thresholdId, testValue);
  ...
  // Render threshold form (wizard or dialog)
  const renderThresholdForm = () => {
    ...
    const FormContent = () => (
      <Grid container spacing={2}>
          ...
          <>
            ...
            <Grid item xs={12} md={6}>
              <FormControlLabel
                control={
                  <Switch
                    checked={editingThreshold.enabled ?? true}
                    onChange={(e) => setEditingThreshold(prev => prev ? {...prev, enabled: e.target.checked} : null)}
                  />
                }
                label="Enabled"
              />
        ...
        {/* Step 2: Threshold Type & Configuration */}
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      label="Color"
                      type="color"
                      value={editingThreshold.color || SEVERITY_COLORS[editingThreshold.severity || 'warning']}
                      onChange={(e) => setEditingThreshold(prev => prev ? {...prev, color: e.target.value} : null)}
                    />
                  </Grid>
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        value={editingThreshold.style || 'solid'}
                        onChange={(e) => setEditingThreshold(prev => prev ? {...prev, style: e.target.value as any} : null)}
                        label="Line Style"
                      >
                  ...
                  <Grid item xs={12}>
                    <Box>
                      <FormControlLabel
                        control={
                          <Switch
                            checked={editingThreshold.fill || false}
                            onChange={(e) => setEditingThreshold(prev => prev ? {...prev, fill: e.target.checked} : null)}
                          />
                        }
                        label="Fill Area"
                      />
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            checked={editingThreshold.showLabel !== false}
                            onChange={(e) => setEditingThreshold(prev => prev ? {...prev, showLabel: e.target.checked} : null)}
                          />
                        }
                        label="Show Label"
                      />
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            checked={editingThreshold.showValue !== false}
                            onChange={(e) => setEditingThreshold(prev => prev ? {...prev, showValue: e.target.checked} : null)}
                          />
                        }
                        label="Show Value"
                      />
          ...
          <>
            <Grid item xs={12}>
              ...
              <Alert severity="info" sx={{ mb: 2 }}>
                <Typography variant="body2">
                  Preview: A threshold named "{editingThreshold.name}" will be created for metric "{editingThreshold.metricName}" 
                  with {editingThreshold.type} type and {editingThreshold.severity} severity.
                </Typography>
              </Alert>
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                helperText="Enter a value to test against this threshold"
              />
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              <Box>
                <Button
                  startIcon={<TestIcon />}
                  onClick={() => editingThreshold.id && handleTestThreshold(editingThreshold.id)}
                  disabled={!editingThreshold.id || loading}
                  variant="outlined"
                  fullWidth
                >
                  Test Threshold
                ...
                {testResult && (
                  <Alert severity={testResult.alertTriggered ? 'warning' : 'success'} sx={{ mt: 1 }}>
                    {testResult.message || (testResult.alertTriggered ? 'Alert would be triggered' : 'No alert would be triggered')}
                  </Alert>
                )}
              </Box>
            </Grid>
    ...
    if (showWizard) {
      return (
        ...
        >
          <DialogTitle>
            Create Threshold - Step {wizardStep + 1} of {WIZARD_STEPS.length}
          </DialogTitle>
  ...
  return (
    <Box className={className}>
        ...
        <>
                ...
                <Grid item xs={12} md={3}>
                  <Box display="flex" gap={1} alignItems="center">
                    ...
                    <Button
                      ...
                      disabled={!bulkOperation || selectedThresholds.size === 0 || loading}
                      variant="outlined"
                      size="small"
                    >
          ...
          <Grid container spacing={2}>
            {filteredThresholds.map(threshold => (
              <Grid item xs={12} md={viewMode === 'grid' ? 6 : 12} key={threshold.id}>
                <Card 
                  variant={threshold.enabled ? "elevation" : "outlined"}
                  sx={{
                    borderLeft: `4px solid ${threshold.color || SEVERITY_COLORS[threshold.severity]}`,
      ...
      {/* Templates Tab Content - Similar structure with template-specific features */}
      {tabValue === 1 && (
        <Grid container spacing={2}>
        ...
        >
          ...
          <DialogActions>
            ...
            <Button 
              ...
              variant="contained"
              disabled={selectedTemplate.requiredVariables.some(v => !templateVariables[v]) || loading}
            >
